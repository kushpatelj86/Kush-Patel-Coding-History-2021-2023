...
class Entry <K,V>
    {
        ...
        //we aren't overriding the object equals method
        //no typecasting is necessary
...
// Designated constructor
  ...
  //return the number of elements currently inside the hash-table
    ...
    //converts a hash table to an index. Essentially, this strips the
    //negative sign and places the hash table in the domain [0,capacity)
    private int normalizeIndex(int keyHash)
    {
        return (keyHash & 0x7FFFFFFF) % capacity;
    }

    //clears all the contents of the has table
    public void clear()
    {
    ...
    //return true/false depending on wheter a key is in the hash table
    ...
    //removes an entry from the given bucket if it exists
    ...
    //inserts an entry in the given bucket only if the entry doesnt already exist in the given bucket
    //if it does then update the entry value
    private V bucketInsertEntry(int bucketIndex, Entry<K,V> entry)
    {
        ...
        if(existentEntry == null)
        {
            ...
            return null; //use null to indicate there was no previous entry
        }
        else
        {
    ...
    //finds and return a partcially entry in a given bucket if it exits, return null otherwise
    ...
    private void resizeTable()
    {
        ...
        for(int i = 0; i < table.length;i++)
        {
            if(table[i] != null)
            {
                ...
                //avoid memeory leak, help the GC
                table[i].clear();
                table[i] = null;
