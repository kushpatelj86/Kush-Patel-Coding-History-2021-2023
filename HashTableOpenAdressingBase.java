...
public abstract class HashTableOpenAdressingBase <K,V> implements Iterable<K>
{
    //global variables
    ...
    //used buckets count the total number of used buckets inside the hash table(includes cells marked as delted)
    //tracks the number of unique keys inside the hash table
    ...
    private static final double DELETE_LOAD_FACTOR = 0.65;


    protected HashTableOpenAdressingBase()
    {
        this(DEFAULT_CAPACITY,DELETE_LOAD_FACTOR);
    }



    protected HashTableOpenAdressingBase(int capacity)
    {
        this(capacity,DELETE_LOAD_FACTOR);
    }
...
protected HashTableOpenAdressingBase(int capacity, double loadFactor) 
  {
    if (capacity <= 0) 
    {
...
//returns true/false depending on wheter the has table is empty
...
public List<K> keys()
{
    List<K> hashtableKeys = new ArrayList<K>();
    for(int i = 0; i < capacity;i++)
    {
